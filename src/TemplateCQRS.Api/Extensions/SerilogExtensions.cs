...
public static class SerilogExtensions
{
    ...
    /// <summary>
    ///     Configures the Serilog logger with specified settings from the provided configuration.
    ///     This includes setting the log output path, defining the minimum log event level,
    ///     enriching the log entries with additional context information,
    ///     and configuring output to both the console and a file.
    /// </summary>
    /// <param name="config">The IConfiguration instance to fetch configuration settings from.</param>
    /// <returns>A LoggerConfiguration instance configured with the specified settings.</returns>
